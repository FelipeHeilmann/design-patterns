...
export class RoomRepositoryDatabase implements IRoomRepository {
    ...
    async getAvailableRoomsByPeriodAndCategory(checkinDate: Date, checkoutDate: Date, category: string): Promise<Room[]> {
        ...
        const roomsData = await connection.query(`select * from design_patterns.room where category = $1 and status = 'available' and room_id not in 
        (select room_id from design_patterns.booking where (checkin_date, checkout_date) overlaps ($2, $3) and status = 'confirmed')`, [category, checkinDate, checkoutDate])
